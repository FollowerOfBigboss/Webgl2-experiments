...
function main() {
	...
	function render(now)
	{
		...
		let transform2 = glMatrix.mat4.create();		
		glMatrix.mat4.translate(transform2, transform2, glMatrix.vec3.fromValues(-0.5, 0.5, 0.0));
		const scaleAmount = Math.sin(Date.now() / 1000);
		glMatrix.mat4.scale(transform2, transform2, glMatrix.vec3.fromValues(scaleAmount, scaleAmount, scaleAmount))	
	
		gl.uniformMatrix4fv(loc, false, transform2);	
		gl.bindVertexArray(VAO);
		gl.drawElements(gl.TRIANGLES, 6, gl.UNSIGNED_INT, 0);
