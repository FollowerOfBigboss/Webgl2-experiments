...
function main() {
	...
	var lightingShader  = new Shader(gl, "materials_vs", "materials_fs");
	...
	function render(now)
	{
		...
		let SinX = Math.sin(currentFrame * 2.0);
		let SinY = Math.sin(currentFrame * 0.7);
		let SinZ = Math.sin(currentFrame * 1.3);
		let lightColor = glMatrix.vec3.fromValues(SinX, SinY, SinZ);
		
		const hVec = glMatrix.vec3.fromValues(0.5, 0.5, 0.5);
		const qVec = glMatrix.vec3.fromValues(0.2, 0.2, 0.2);

		let diffuseColor = glMatrix.vec3.create();
		glMatrix.vec3.mul(diffuseColor, lightColor, hVec);	
		let ambientColor = glMatrix.vec3.create();
		glMatrix.vec3.mul(ambientColor, diffuseColor, qVec);
